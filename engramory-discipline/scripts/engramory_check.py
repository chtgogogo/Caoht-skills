...
On hosts with a PreToolUse hook the cap is enforced deterministically on every edit.
On any other host that lacks such a hook, the agent should run THIS after writing
the index, and compact if it says OVER:

    python engramory_check.py <path-to-MEMORY.md>
