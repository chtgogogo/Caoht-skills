...
engramory_doctor — consistency + protocol check for an Engramory memory store.
    ...
    python engramory_doctor.py <MEMORY_ROOT>              # structure + schema
    python engramory_doctor.py <MEMORY_ROOT> --no-schema  # structure only
...
(exit 0): a `name` not matching the filename slug (tolerating `-`/`_`/case; a non-ASCII
e.g. CJK name is treated as a deliberate human title and is NOT compared to its ASCII
slug), and a note reachable only via a `[[wikilink]]` (not in the index, so it won't
load at session start). Broken `[[wikilinks]]` are INFO (forward-reference stubs allowed).
...
__ENGRA_DOC = """
engramory_doctor checks an Engramory store for drift the per-write checks miss.
Run it periodically (or in CI) on your MEMORY_ROOT:

    python engramory_doctor.py <MEMORY_ROOT>

It reports ISSUE (exit 1) for: over-cap index, broken/missing pointers, symlink
escapes, duplicate slugs, orphan notes, and schema violations (missing name/
description/type/created/updated, invalid type, feedback/project missing Why:/How
to apply:). Add --no-schema to check structure only.
"""
...
def main(argv):
    ...
    if "-h" in argv[1:] or "--help" in argv[1:]:
        print((__doc__ or __ENGRA_DOC or "").strip())
        return 0
    schema = "--no-schema" not in argv[1:]  # default: validate frontmatter too
    ...
    for base, p in sorted(notes.items()):
        ...
        if fm is None and not fm_problems:
            ...
            name = fm.get("name", "")
            if name:
                # A non-ASCII name (e.g. CJK) is a deliberate human-facing title; its
                # slug stays an ASCII machine ID by design, so the name<->slug mismatch
                # is expected and must NOT be flagged. Convention (see user MEMORY.md):
                # the human reader doesn't read English, so `name` is Chinese and the
                # filename is an English slug — only ASCII names are compared to slug.
