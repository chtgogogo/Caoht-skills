...
## 目录升格（2026-09-23）
- 做了什么：权威库目录升格 Cao- → A-Cao-资深工程师五模板-构建审查根因架构计划，标记为常驻注册类；SKILL.md 无功能变更。
- 解决了什么：它与编码行为准则同为「任务专用宁多触发」纪律型高频模板且已注册常驻，命名对齐 A- 规范。
- 怎么解决：目录改名 + 映射表（md/csv）+ 命名规则 + patrol_check PAIRS 同步。
